...
select 
        ...
        case
        when three_sub_index_criteria(pm25_avg,pm10_avg,so2_avg,no2_avg,nh3_avg,co_avg,o3_avg) > 2 then greatest 
        (get_int(pm25_avg),get_int(pm10_avg),get_int(so2_avg),get_int(no2_avg),get_int(nh3_avg),get_int(co_avg),get_int(o3_avg))
        else 0
        end
        as aqi
